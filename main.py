...
if args.do_eval:
    ...
    dev_sents, _ = read_line_examples_from_file(
        f"data/{args.task}/{args.dataset}/dev.txt"
    )
    test_sents, _ = read_line_examples_from_file(
    ...
    for checkpoint in all_checkpoints:
        ...
        if 0 <= int(epoch) < 100:
            ...
            dev_result, _ = evaluate(
                dev_loader, model, args.paradigm, args.task, dev_sents
            )
            ...
            test_result, _ = evaluate(
                test_loader, model, args.paradigm, args.task, test_sents
            )
            test_result = dict(
...
if args.do_direct_eval:
    ...
    raw_scores, fixed_scores = evaluate(
        test_loader, model, args.paradigm, args.task, sents
    )
    # print(scores)
    ...
    exp_results = (
        f"Raw F1 = {raw_scores['f1']:.4f}, Fixed F1 = {fixed_scores['f1']:.4f}"
    )
    log_str = f"============================================================\n"
